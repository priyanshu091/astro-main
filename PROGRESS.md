...
- **Bugfix**: Fixed the frontend "PICK DATE" button not opening the native calendar popup. Attached the `showPicker()` API directly to the input's `onClick` handler so clicking anywhere on the invisible input correctly brings up the date selection calendar across all modern browsers.
